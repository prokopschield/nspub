...
(async () => {
	...
	do {
		try {
				...
				do {
					dir =
						...
						(await new Promise((resolve) =>
							paste((error, content) => resolve(content))
						));
			...
			cont =
				(await ask('Upload another? (yes/no)'))?.toLocaleLowerCase()[0] === 'y';
		} catch (error) {
			cont = false;
		}
	} while (cont);
